...
DEFAULT_SESSION_DIR = Path.home() / ".task-mind" / "sessions"


def get_session_base_dir() -> Path:
    """Get session storage base directory

    Supports customization via environment variable TASK_MIND_SESSION_DIR.

    Returns:
        Session storage base directory path
    """
    custom_dir = os.environ.get("TASK_MIND_SESSION_DIR")
    if custom_dir:
