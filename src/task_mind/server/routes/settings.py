...
async def get_main_config() -> MainConfigResponse:
    ...
    from task_mind.init.configurator import PRESET_ENDPOINTS
    ...
    return MainConfigResponse(
        working_directory=config.get("working_directory_display", "~/.task-mind"),
...
async def update_auth(request: AuthUpdateRequest) -> ApiResponse:
    ...
    from task_mind.init.models import APIEndpoint
...
async def open_working_directory() -> ApiResponse:
    ...
    try:
        config = MainConfigService.get_config()
        working_dir = config.get("working_directory", os.path.expanduser("~/.task-mind"))
