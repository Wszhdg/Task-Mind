...
export async function startAgentTask(
  ...
  const task = await httpApi.startAgent(prompt, projectPath);
  return {
    status: 'ok',
...
export async function continueAgentTask(
  ...
  // This calls task-mind agent --resume {session_id} on the backend
...
export async function checkConnection(): Promise<ConnectionStatus> {
  ...
  try {
    ...
    return {
      connected: true,
      message: 'Connected to Task-Mind web service',
    };
  } catch {
    return {
      connected: false,
      message: 'Failed to connect to Task-Mind web service',
    };
  }
}
